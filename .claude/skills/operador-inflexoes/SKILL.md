...
description: Executa o Operador de Inflexões — perfil epistemológico habitável (validado por 11 testes + E2E) que produz teses de descoberta de ações ao nível de DD de topo, lê gargalos em 2.ª ordem, triangula compromissos custosos e fecha em recomendação por lógica de tese (nunca score), com fronteira dura descoberta≠execução. Usar sempre que o utilizador diga "corre o operador", "operador de inflexões", "tese de descoberta", "descobre uma inflexão/tema", "mapa de portagens", "compromissos custosos", "corrida do T1", "corrida de inflexões", "kit ChatGPT do operador", "par de variância", ou peça uma tese causal profunda sobre uma transformação económica com veículos cotados — mesmo sem nomear o skill. Três workflows - descoberta (corrida do operador), corrida T1 (longitudinal semanal do motor), kits ChatGPT. NÃO usar para - Kullamägi/KMG, Entre-Linhas, Jackson, Alpha-K, Kristjan Deep, nem scans/validação técnica de breakouts — têm skills próprios.
---
